...
//생년월일 입력 후 '계산' 버튼 클릭 시 생일부터 지금까지 몆일이 지났는지, 몇시간이 지났는지 보여주는 프로그램(양력기준)
// 지금까지 흐른 날짜 계산법 : 1000*60*60*24로 나누기
// 지금까지 흐른 시간 계산법 : 1000*60*60로 나누기 
const birthDate = document.querySelector('#birthdate');
const birthBtn = document.querySelector('#birth_btn');
const resultDate = document.querySelector('.result .date em');
const resultTime = document.querySelector('.result .time em');
